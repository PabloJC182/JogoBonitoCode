...
public class IDECompilador extends JFrame {
    ...
    private void initComponents() {
        ...
        btnCompilar    = crearBotonEstilizado(" Compilar",              new Color(46, 125, 50));
        btnTokens      = crearBotonEstilizado(" Tokens",                new Color(25, 118, 210));
        btnErrores     = crearBotonEstilizado(" Errores",               new Color(211, 47, 47));
    ...
    private void btnCompilarActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try {
            ...
            AnalizadorSemantico.limpiar();   // tabla de símbolos
            ...
            ReporteCodigo3D.generar();
            ReporteTablaSimbolos.generar();

            if (TablaGlobal.errores.isEmpty()) {
                iluminarSintaxis();
                JOptionPane.showMessageDialog(this,
                        "¡Juego Limpio! Compilación exitosa.\nTokens generados: " + TablaGlobal.tokens.size(),
                        "Resultado del VAR", JOptionPane.INFORMATION_MESSAGE);
            } else {
                for (modelos.ErrorCompilador err : TablaGlobal.errores) {
                    if (err.linea > 0) resaltarError(err.linea);
                }
                JOptionPane.showMessageDialog(this,
    ...
    private void iluminarSintaxis() {
        try {
            ...
            while (mCiclos.find())
                highlighter.addHighlight(mCiclos.start(), mCiclos.end(),
                        new DefaultHighlighter.DefaultHighlightPainter(new Color(173, 216, 230, 100)));
            while (mRelato.find())
